...
namespace cg::renderer
{
												...
												size_t in_height)
	{
		width = in_width;
		height = in_height;
		history = std::make_shared<cg::resource<float3>>(
				width, height);
	}
...
#pragma omp parallel for
	...
	inline void aabb<VB>::add_triangle(const triangle<VB> triangle)
	{
		...
		aabb_min = min(aabb_max, triangle.a);
		aabb_min = min(aabb_max, triangle.b);
		aabb_min = min(aabb_max, triangle.c);
	}
	...
	{
		float3 inv_ray_direction = 1.f / ray.direction;
